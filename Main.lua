...
local function G(H,I)
    ...
    for K,L in pairs(I)do J[K]=L end; table.insert(o,J) return J 
end
...
local function ac()
    ...
    local af=Instance.new("CanvasGroup",ae);af.Size=UDim2.new(0,500,0,450);af.Position=UDim2.new(0.5,-250,0.5,-225);af.BackgroundColor3=Color3.fromRGB(15,15,15);af.Visible=false;af.GroupTransparency=1;Instance.new("UICorner",af).CornerRadius=UDim.new(0,12);local ag=Instance.new("UIStroke",af);ag.Thickness=2;ag.Color=_G.ESP_Color
    ...
    local ai=Instance.new("TextLabel",af);ai.Size=UDim2.new(1,0,0,50);ai.Text="CHEOZ MOBILE V2";ai.TextColor3=_G.ESP_Color;ai.Font=Enum.Font.LuckiestGuy;ai.TextSize=25;ai.BackgroundTransparency=1

    local function ak(am)
        local an=Instance.new("Frame",af);an.Size=UDim2.new(0.45,0,0.72,0);an.Position=UDim2.new(am,0,0.12,0);an.BackgroundColor3=Color3.fromRGB(22,22,22);Instance.new("UICorner",an)
        local ap=Instance.new("Frame",an);ap.Size=UDim2.new(0.9,0,0.9,0);ap.Position=UDim2.new(0.05,0,0.05,0);ap.BackgroundTransparency=1;local ui=Instance.new("UIListLayout",ap);ui.Padding=UDim.new(0,6);ui.HorizontalAlignment="Center";return ap
    end
    ...
    local function createBtn(parent, text, var)
        local b = Instance.new("TextButton", parent);b.Size=UDim2.new(1,0,0,35);b.Text=text;b.BackgroundColor3=Color3.fromRGB(35,35,35);b.TextColor3=Color3.new(1,1,1);b.Font=Enum.Font.GothamBold;b.TextSize=11;Instance.new("UICorner",b)
    ...
    local function createStepper(parent, text, var, inc, min, max)
        ...
        local l = Instance.new("TextLabel", f); l.Size = UDim2.new(1,0,0,15); l.Text = text..": "..tostring(_G[var]); l.TextColor3 = Color3.new(1,1,1); l.BackgroundTransparency = 1; l.Font = Enum.Font.Gotham; l.TextSize = 10
        local btnM = Instance.new("TextButton", f); btnM.Size = UDim2.new(0,30,0,25); btnM.Position = UDim2.new(0.1,0,0.4,0); btnM.Text = "-"; btnM.BackgroundColor3 = Color3.fromRGB(40,40,40); btnM.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", btnM)
        local btnP = Instance.new("TextButton", f); btnP.Size = UDim2.new(0,30,0,25); btnP.Position = UDim2.new(0.7,0,0.4,0); btnP.Text = "+"; btnP.BackgroundColor3 = Color3.fromRGB(40,40,40); btnP.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner", btnP)
        
        btnM.MouseButton1Click:Connect(function() _G[var] = math.max(min, _G[var] - inc); l.Text = text..": "..string.format("%.1f", _G[var]) end)
        btnP.MouseButton1Click:Connect(function() _G[var] = math.min(max, _G[var] + inc); l.Text = text..": "..string.format("%.1f", _G[var]) end)
    end
    ...
    createStepper(aq, "DELAY TIME", "DelayTime", 0.1, 0, 5)
    createStepper(aq, "SMOOTHNESS", "Smoothness", 0.1, 0.1, 2)
...
b.RenderStepped:Connect(function()
    ...
    if _G.AimbotEnabled then
        ...
        if target and target.Character then
            ...
            t = k() -- Reseta o timer do Kill Delay se não tiver target
        end
    end
end)
pcall(ac)
